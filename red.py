...
def banner():
    ...
    print(u"  |      /  %%%%%%%%%%%%  \     | | | | |                  |"); time.sleep(delay_per_line)
    print(u"  |     /  %%%%%%%%%%%%%%  \    | | | | |    release by: red {}{}{}  |".format(appname_color,(__version__)[0:3],color.END)); time.sleep(delay_per_line)
    ...
    print(u'  | [{}1{}] Limpe todo o espaço livre em uma partição escolhida         |'.format(options_color,color.END)); time.sleep(delay_per_line)
    print(u'  | [{}2{}] Limpe um único arquivo / todos os arquivos em um caminho escolhido  |'.format(options_color,color.END)); time.sleep(delay_per_line)
    print(u'  +-- {}MODO DE PESQUISA AUTOMÁTICA{} ------------------------------------+'.format(headers_color,color.END)); time.sleep(delay_per_line)
    print(u'  | [{}3{}] Limpe meu diretório pessoal                         |'.format(options_color,color.END)); time.sleep(delay_per_line)
    print(u'  +-- {}MODO DE PESQUISA AUTOMÁTICA (elevated privileges){} --------------+'.format(headers_color,color.END)); time.sleep(delay_per_line)
    print(u'  | [{}4{}] Limpe todos os diretórios pessoais dos usuários               |'.format(options_color,color.END)); time.sleep(delay_per_line)
    print(u'  | [{}5{}] Limpe todas as partições de swap / arquivos de paginação         |'.format(options_color,color.END)); time.sleep(delay_per_line)
    print(u'  +-- {}MÉTODOS DE SUBSTITUIÇÃO{} -----------------------------------+'.format(headers_color, color.END));time.sleep(delay_per_line)
    print(u'  | [{}6{}] Alterar método de substituição                            |'.format(options_color, color.END));time.sleep(delay_per_line)
    print(u'  +--------------------------------------------------------+'); time.sleep(delay_per_line)
    print(u'  | [{}0{}] Saida                                               |'.format(options_color,color.END)); time.sleep(delay_per_line)
...
def set_root_path():
    print(u'  {}[!]{} Sistema detectado: {}{}{} (running {}{}{}) '.format(color.ORANGE, color.END,
    ...
    print(u'  {}[!]{} Para limpar um sistema não inicializado, você pode fornecer a raiz'.format(color.ORANGE,color.END))
    print(u'  {}[!]{} caminho onde é montado(e.g.: /media/mydrive).'.format(color.ORANGE, color.END))
    print(u'  {}[!]{} Defina um caminho de raiz personalizado ou PRESSIONE ENTER para continuar...'.format(color.ORANGE, color.END))
    root_path = False
    ...
    if (os.name != 'posix' and os.name != 'nt'):
        ...
        while True:
            ...
            if interactive:
                ...
                path = ''
                opt = input(u'\r\n  {}[?]{} Escolha uma opção: (help to show again) [{}0-9{}]: '.format(color.PURPLE,color.END,color.ORANGE,color.END))
            ...
            if (opt == '1' or wipe_free_arg):
                ...
                if wipe_free_arg:
                    ...
                    path = input(u'  {}[?]{} Digite o caminho que deseja limpar todo o espaço livre: '.format(color.PURPLE,color.END))
                status = wiper_ops.wipe_free_space(path, method)
                if status == 0:
                    print(u'  {}[+]{} O espaço livre foi limpo com sucesso.'.format(color.GREEN, color.END))
                else:
                    print(u'  {}[!]{} ocorreu um erro'.format(color.RED, color.END))
